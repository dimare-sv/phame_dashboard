...
export const LAYER_SPECS: LayerSpec[] = [
  /* ================================ 01 획득 ================================ */
  {
    ...
    subs: [
      ...
      /* "SNS 간편가입 비중"(2026-09-17 삭제): 벤치마크 없는 단일 비율이라 활용도가
         낮다는 기획 피드백 — 아래 "가입방식" 축 분해(카카오·애플·네이버·이메일,
         항목별 비중+전기 대비)가 이미 같은 정보를 더 자세히 준다. */
      ...
      /* "초대 코드 복사"(2026-09-17 변경): 복사 건수 단독으론 바이럴 효과를 못 읽는다는
         기획 피드백 — 복사 대비 실제 가입 완료로 이어진 비율로 바꾼다. */
      { name: "초대가입 전환율", kind: "rate", v: 24.4, vp: [23.1, 24.4, 26.8], d: [1.1, 1.3, 2.7], metricId: "acq-07" },
    ],
    targets: [
  ...
  /* ================================ 02 활성 ================================ */
  {
    ...
    subs: [
      ...
      /*
       * 지금까지 서브 지표가 전부 수요자(파머) 관점이라 공급자(마스터)가 콘텐츠를
       * 얼마나 생산하는지 볼 지표가 없었다(2026-09-17 기획 피드백) — 지표사전에
       * 이미 있던 "피드 신규 등록 수"(active-07, UGC 생산량)를 마스터 관점 서브
       * 지표로 노출한다. dictionary.ts 에 적어둔 대로 GA4엔 등록 이벤트가 없고
       * DB feeds.created_at 만 있다 — 아래 수치는 DB 연동 전 예시.
       */
      {
        name: "피드 신규 등록 수",
        kind: "count",
